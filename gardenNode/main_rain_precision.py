...
def round_weather_fields(obs):
    ...
    return {
        "ts": int(obs[0]),
        "t": round(float(obs_value(7, 0)), 1),
        "h": int(round(float(obs_value(8, 0)))),
        "p": round(float(obs_value(6, 0)), 1),
        "w": round(float(wind_speed), 1),
        "g": round(float(obs_value(3, wind_speed)), 1),
        "l": round(float(obs_value(1, wind_speed)), 1),
        "d": int(round(float(wind_direction))),
        "r": raw_float(obs_value(12, 0)),
        "uv": round(float(obs_value(10, 0)), 1),
        "sr": round(float(obs_value(11, 0)), 1),
        "lux": int(round(float(obs_value(9, 0)))),
        "bat": round(float(obs_value(16, 0)), 2),
        "ld": round(float(obs_value(14, 0)), 1),
        "lc": int(round(float(obs_value(15, 0)))),
        "pt": int(round(float(obs_value(13, 0)))),
        "ri": int(round(float(obs_value(17, 0)))),
        "rd": raw_float(obs_value(18, 0)),
        "nr": raw_float(obs_value(19, 0)),
        "nrd": raw_float(obs_value(20, 0)),
        "pa": int(round(float(obs_value(21, 0)))),
    }
...
def send_obs_st_to_rak(weather):
    ...
    for key in ("g", "l", "uv", "sr", "lux", "bat", "ld", "lc", "pt", "ri",
                "rd", "nr", "nrd", "pa"):
        value = weather[key]
